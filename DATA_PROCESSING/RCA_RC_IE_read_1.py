...
df_count = utils.calculate_kmeans(df_count, 'count_total')
...
for abbrv in list_abbrv:
    ...
    gdf_tract['Event_Count_{}'.format(abbrv)] = np.repeat(df_count.loc[df_count.abbrv == abbrv, 'count_total'].values[0],
                                                    len(gdf_tract))
