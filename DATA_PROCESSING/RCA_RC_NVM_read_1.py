...
# No need to rerun.  The API outputs were saved and are loaded in next code block.
# for i, idx in enumerate(gdf_tract.index):
#     lat = str( np.round(gdf_tract['lat'].iloc[i], 5))
#     lon = str(np.round(gdf_tract['lon'].iloc[i], 5))
...
#     print(i)
#
# path_save = r'C:\temp\Scratch\NYC_walkscore.csv'
# gdf_tract[['BCT_txt', 'walkscore', 'lat', 'lon']].to_csv(path_save)

#%% load walkscore and merge to tract shapefile
path_walkscore = params.PATHNAMES.at['RCA_RC_NVM_raw', 'Value']
...
#%% save as output
path_output = params.PATHNAMES.at['RCA_RC_NVM_score', 'Value']
...
print("Finished calculating RCA factor: non-vehicle mobility.")