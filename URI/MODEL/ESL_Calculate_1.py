...
def calculate_ESL(haz):
    ...
    for idx in df_cons.index:
        ...
        filtered_ESL = gdf_ESL[gdf_ESL[raw_col] > 0]
        filtered_ESL[normalized_col] = filtered_ESL.apply(
            lambda row: utils.divide_zero(row[raw_col], row[receptor_field]), axis=1)
        filtered_ESL = utils.calculate_percentile(filtered_ESL, data_column=normalized_col, score_column=percentile_col)
        filtered_ESL = utils.calculate_kmeans(filtered_ESL, data_column=normalized_col, score_column=score_col)
        gdf_ESL = gdf_ESL.merge(filtered_ESL[['BCT_txt', normalized_col, score_col, percentile_col]], on='BCT_txt', how='left')
    ...
    filtered_ESL = gdf_ESL[gdf_ESL[percentile_sum_col] > 0]
    filtered_ESL = utils.calculate_percentile(filtered_ESL, data_column=percentile_sum_col, score_column=final_percentile_col)   
    filtered_ESL = utils.calculate_kmeans(filtered_ESL, data_column=score_sum_col, score_column=final_score_col)
    # Fill na with 0 in gdf_ESL
    gdf_ESL = gdf_ESL.merge(filtered_ESL[['BCT_txt', final_score_col, final_percentile_col]], on='BCT_txt', how='left')
    gdf_ESL = gdf_ESL.fillna(0)
