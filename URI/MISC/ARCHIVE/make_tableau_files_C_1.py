...
gdf_shp['Citywide'] = np.repeat(int(1), len(gdf_shp))#  np.ones(len(gdf_uri))
gdf_shp['Bldg_Area'] = gdf_shp['Floor_sqft']
#save the most important fields
gdf_shp = gdf_shp[['BCT_txt', 'NEIGHBORHO', 'PUMA', 'BOROCODE', 'Citywide', 'Sq_miles',
...
gdf_shp = gdf_shp[['Census_Tract', 'NTA_Name', 'PUMA', 'Boro_Code', 'Boro_Name', 'Citywide',
                   'Land_Area', 'Pop_2010', 'Bldg_Count', 'Bldg_Area', 'geometry']]
gdf_tract = gdf_shp.copy()

#aggregate date first
df_data= gdf_shp.groupby(["Citywide"]).agg(Pop_2010=("Pop_2010", sum),
                                             Bldg_Count=("Bldg_Count", sum),
                                             Bldg_Area=("Bldg_Area", sum),
                                             Land_Area=("Land_Area", sum))
#dissolve the geometry
gdf_shp.geometry = gdf_shp.buffer(0)  #repear border
gdf_shp  = gdf_shp.dissolve(by='Citywide')
gdf_shp = gdf_shp[['geometry']].merge(df_data, left_index=True, right_on='Citywide', how='left')
gdf_shp['Citywide'] = gdf_shp.index
gdf_shp.index.name = 'Index'
path_shp = params.PATHNAMES.at['TBL_C_shp', 'Value']
...
gdf_SOV = gdf_SOV.merge(gdf_tract[['Census_Tract', 'Citywide', 'Pop_2010']], left_on='BCT_txt', right_on='Census_Tract', how='left')
...
gdf_SOV = gdf_SOV.groupby(["Citywide"]).agg(SOV=("SOV", wm))
gdf_SOV['Citywide'] = gdf_SOV.index
gdf_SOV.index.name = 'Index'
df_tract = gdf_SOV[['Citywide','SOV']].merge(gdf_shp[[ 'Citywide',
                   'Land_Area', 'Pop_2010', 'Bldg_Count', 'Bldg_Area']], left_on='Citywide',
                                          right_on='Citywide', how='left')
df_tract.rename(columns={'SOV':'Value'}, inplace=True)
df_tract['Geography'] = np.repeat('Citywide', len(df_tract))
...
def update_tract(df_master, df_base, df_new, value_column, hazard=nan_value, component=nan_value,
                 ...
                 base_key='Citywide', new_key='Citywide'):
    df = df_new[[new_key, value_column]].merge(df_base[['Land_Area', 'Pop_2010', 'Bldg_Count', 'Bldg_Area', 'Citywide']], left_on=new_key, right_on=base_key, how='left')
    #df.drop(columns=[new_key], inplace=True)
    df.rename(columns={value_column: 'Value'}, inplace=True)
    df['Geography'] = np.repeat('Citywide', len(df))
...
list_hazards = ['CYB', 'EXH', 'WIW', 'CST', 'CER', 'HIW', 'ERQ', 'FLD', 'EMG', 'RES', 'CRN', ]
...
for i in np.arange(len(list_hazards)):
    ...
    gdf_RCA['Citywide'] = np.repeat(int(1), len(gdf_RCA))#  np.ones(len(gdf_uri))
    gdf_RCA = gdf_RCA.merge(gdf_tract[[ 'Pop_2010', 'Census_Tract']], left_on='BCT_txt', right_on='Census_Tract', how='left')
    wm = lambda x: weight_ave(x, weights=gdf_RCA.loc[x.index, "Pop_2010"])
    gdf_RCA  = gdf_RCA.groupby(["Citywide"]).agg(wm)
    gdf_RCA.index.name = 'Index'
    gdf_RCA['Citywide'] = gdf_RCA.index
...
for i in np.arange(len(list_hazards)):
    ...
    gdf_uri['Citywide'] = np.repeat(int(1), len(gdf_uri))#  np.ones(len(gdf_uri))
    #gdf_uri['Boro_Code'] = gdf_uri['BOROCODE']
    wm = lambda x: weight_ave(x, weights=gdf_uri.loc[x.index, "pop_2010"])
    gdf_temp = gdf_uri.groupby(["Citywide"]).agg(wm)
    gdf_uri = gdf_uri.groupby(["Citywide"]).agg(sum)
    gdf_uri['Citywide'] = gdf_uri.index
    ...
    #add back in
    gdf_uri = gdf_uri.merge(gdf_temp[['SOV','RCA']], left_on='Citywide', right_index=True, how='left')
    gdf_uri['Citywide'] = np.repeat(int(1), len(gdf_uri))#  np.ones(len(gdf_uri))
...
#%% calculate ESL for subcomponents
    ...
    gdf_uri = gpd.read_file(path_haz)
    gdf_uri['Citywide'] = np.repeat(int(1), len(gdf_uri))#  np.ones(len(gdf_uri))
    #gdf_uri['Citywide'] = gdf_uri['Citywide']
    wm = lambda x: weight_ave(x, weights=gdf_uri.loc[x.index, "pop_2010"])
    gdf_temp = gdf_uri.groupby(["Citywide"]).agg(wm)
    gdf_uri = gdf_uri.groupby(["Citywide"]).agg(sum)
    gdf_uri['Citywide'] = gdf_uri.index
...
for norm in list_norm:
    ...
    df_all = df_uri[['Value', 'Citywide']].groupby(by='Citywide').sum()
    df_all['Citywide'] = df_all.index
...
#%% save file
path_tracts = r'C:\temp\TBL_CITYWIDE_plusCYB_v1.csv' #params.PATHNAMES.at['TBL_tracts', "Value"]
...
#     gdf_shp =  gdf_shp.merge(df_plot[['Boro_Code', 'Value']], on='Boro_Code', how='left')
...
#     plt.savefig(r'C:\temp\Tableau_Tables_BORO\{}_{}_{}_{}_{}_{}_{}_{}.png'.format(Geography, Hazard, Component, Subcomponent, Factor, View, Type, Normalization), npi=100)
