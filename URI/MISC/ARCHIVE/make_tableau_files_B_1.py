...
df_data= gdf_shp.groupby(["Boro_Code"]).agg(Pop_2010=("Pop_2010", sum),
...
gdf_shp  = gdf_shp.dissolve(by='Boro_Code')
gdf_shp = gdf_shp[['geometry']].merge(df_data, left_index=True, right_on='Boro_Code', how='left')
gdf_shp['Boro_Code'] = gdf_shp.index
gdf_shp.index.name = 'Index'
path_shp = params.PATHNAMES.at['TBL_B_shp', 'Value']
...
gdf_SOV = gdf_SOV.merge(gdf_tract[['Census_Tract', 'Boro_Code', 'Pop_2010']], left_on='BCT_txt', right_on='Census_Tract', how='left')
...
gdf_SOV = gdf_SOV.groupby(["Boro_Code"]).agg(SOV=("SOV", wm))
gdf_SOV['Boro_Code'] = gdf_SOV.index
gdf_SOV.index.name = 'Index'
df_tract = gdf_SOV[['Boro_Code','SOV']].merge(gdf_shp[[ 'Boro_Code',
                   'Land_Area', 'Pop_2010', 'Bldg_Count', 'Bldg_Area']], left_on='Boro_Code',
                                          right_on='Boro_Code', how='left')
df_tract.rename(columns={'SOV':'Value'}, inplace=True)
df_tract['Geography'] = np.repeat('Borough', len(df_tract))
...
def update_tract(df_master, df_base, df_new, value_column, hazard=nan_value, component=nan_value,
                 ...
                 base_key='Boro_Code', new_key='Boro_Code'):
    df = df_new[[new_key, value_column]].merge(df_base[['Land_Area', 'Pop_2010', 'Bldg_Count', 'Bldg_Area', 'Boro_Code']], left_on=new_key, right_on=base_key, how='left')
    #df.drop(columns=[new_key], inplace=True)
    df.rename(columns={value_column: 'Value'}, inplace=True)
    df['Geography'] = np.repeat('Borough', len(df))
...
for i in np.arange(len(list_hazards)):
    ...
    gdf_RCA = gdf_RCA.merge(gdf_tract[['Boro_Code', 'Pop_2010', 'Census_Tract']], left_on='BCT_txt', right_on='Census_Tract', how='left')
    wm = lambda x: weight_ave(x, weights=gdf_RCA.loc[x.index, "Pop_2010"])
    gdf_RCA  = gdf_RCA.groupby(["Boro_Code"]).agg(wm)
    gdf_RCA.index.name = 'Index'
    gdf_RCA['Boro_Code'] = gdf_RCA.index
...
for i in np.arange(len(list_hazards)):
    ...
    gdf_uri = gdf_uri.groupby(["BOROCODE"]).agg(sum)
    gdf_uri['Boro_Code'] = gdf_uri.index
    ...
    #add back in
    gdf_uri = gdf_uri.merge(gdf_temp[['SOV','RCA']], left_on='Boro_Code', right_index=True, how='left')
...
for norm in list_norm:
    ...
    df_all = df_uri[['Value', 'Boro_Code']].groupby(by='Boro_Code').sum()
    df_all['Boro_Code'] = df_all.index
...
#%% save file
path_tracts = r'C:\temp\TBL_BORO_v1.csv' #params.PATHNAMES.at['TBL_tracts', "Value"]
