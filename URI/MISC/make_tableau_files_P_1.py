...
df_data= gdf_shp.groupby(["PUMA"]).agg(Pop_2010=("Pop_2010", sum),
...
gdf_shp  = gdf_shp.dissolve(by='PUMA')
gdf_shp = gdf_shp[['geometry']].merge(df_data, left_index=True, right_on='PUMA', how='left')
gdf_shp['PUMA'] = gdf_shp.index
gdf_shp.index.name = 'Index'
path_shp = params.PATHNAMES.at['TBL_P_shp', 'Value']
...
gdf_SOV = gdf_SOV.merge(gdf_tract[['Census_Tract', 'PUMA', 'Pop_2010']], left_on='BCT_txt', right_on='Census_Tract', how='left')
...
gdf_SOV = gdf_SOV.groupby(["PUMA"]).agg(SOV=("SOV", wm))
gdf_SOV['PUMA'] = gdf_SOV.index
gdf_SOV.index.name = 'Index'
df_tract = gdf_SOV[['PUMA','SOV']].merge(gdf_shp[[ 'PUMA',
                   'Land_Area', 'Pop_2010', 'Bldg_Count', 'Bldg_Area']], left_on='PUMA',
                                          right_on='PUMA', how='left')
df_tract.rename(columns={'SOV':'Value'}, inplace=True)
df_tract['Geography'] = np.repeat('PUMA', len(df_tract))
...
def update_tract(df_master, df_base, df_new, value_column, hazard=nan_value, component=nan_value,
                 ...
                 base_key='PUMA', new_key='PUMA'):
    df = df_new[[new_key, value_column]].merge(df_base[['Land_Area', 'Pop_2010', 'Bldg_Count', 'Bldg_Area', 'PUMA']], left_on=new_key, right_on=base_key, how='left')
    #df.drop(columns=[new_key], inplace=True)
    df.rename(columns={value_column: 'Value'}, inplace=True)
    df['Geography'] = np.repeat('PUMA', len(df))
...
for i in np.arange(len(list_hazards)):
    ...
    gdf_RCA  = gdf_RCA.groupby(["PUMA"]).agg(wm)
    gdf_RCA.index.name = 'Index'
    gdf_RCA['PUMA'] = gdf_RCA.index
...
for i in np.arange(len(list_hazards)):
    ...
    gdf_temp = gdf_uri.groupby(["PUMA"]).agg(wm)
    gdf_uri = gdf_uri.groupby(["PUMA"]).agg(sum)
    gdf_uri['PUMA'] = gdf_uri.index
    ...
    #add back in
    gdf_uri = gdf_uri.merge(gdf_temp[['SOV','RCA']], left_on='PUMA', right_index=True, how='left')
...
for norm in list_norm:
    ...
    df_all = df_uri[['Value', 'PUMA']].groupby(by='PUMA').sum()
    df_all['PUMA'] = df_all.index
...
#%% save file
path_tracts = r'C:\temp\TBL_PUMA_v1.csv' #params.PATHNAMES.at['TBL_tracts', "Value"]
...
#     gdf_shp =  gdf_shp.merge(df_plot[['PUMA', 'Value']], on='PUMA', how='left')
...
#     plt.savefig(r'C:\temp\Tableau_Tables_PUMA\{}_{}_{}_{}_{}_{}_{}_{}.png'.format(Geography, Hazard, Component, Subcomponent, Factor, View, Type, Normalization), npi=100)
