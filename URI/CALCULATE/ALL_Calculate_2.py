...
def calculate_ALL(list_abbrev_haz):
    ...
    path_haz_dict = {haz: PATHNAMES.OUTPUTS_folder + fr'\URI\Tract\URI_{haz}_tract.shp' for haz in list_abbrev_haz}
    gdf_haz_dict = {haz: gpd.read_file(path_haz_dict[haz]).set_index('BCT_txt') for haz in path_haz_dict.keys()}
    ...
    list_rntt_series = [gdf_haz_dict[haz][[haz + 'E_RNTT']] for haz in list_abbrev_haz]
    gdf_all['ALLE_RNTT'] = pd.concat(list_rntt_series, axis=1).sum(axis=1)

    #%% cacluate all hazard average RCA
    list_RCA_series = [gdf_haz_dict[haz][[haz + 'R_RTTTT']] for haz in list_abbrev_haz]
    gdf_all['ALLR_RTTTT'] = pd.concat(list_RCA_series, axis=1).mean(axis=1) 

    #%% calcualte all hazard URI
    list_URI_series = [gdf_haz_dict[haz][[haz + 'U_RN']] for haz in list_abbrev_haz]
    gdf_all['ALLU_RN'] = pd.concat(list_URI_series, axis=1).sum(axis=1)

    #%% remove duplicate losses (e.g., high winds)
    # Three hazards have duplicate values
    # ESL_CST_hazus_loss (used in CST and HIW)
    # ESL_CST_dislocation_loss (used iN CST and FLD)
    # ESL_FLD_hazus_loss (used in CST and FLD)

    # However, Arcadis disagrees that these should be removed for many reasons!
    # 1. CST and FLD impacts ~should~ be calculated differently. 
    ...
    normalization_dict = {'A': 'AREA_SQMI',
                          'F': 'FLOOR_SQFT',
                          'P': 'POP'}
    list_norm_col = ['AREA_SQMI', 'FLOOR_SQFT' , 'POP']
    list_abbrv = ['A', 'F', 'P']
    list_col = ['ALLE_RNTT', 'ALLU_RN']
    for abbrv in normalization_dict.keys():
        norm = normalization_dict[abbrv]
        for col in list_col:
